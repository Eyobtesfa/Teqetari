...

{
...

    {
        ...
        var employeeExists = await context.Employees.IgnoreQueryFilters().AnyAsync(e => e.Id == create.EmployerId, ct);
        if (!employeeExists)
        {
            logger.LogError("Employer with ID: {EmployerId} does not exist", create.EmployerId);
            throw new ArgumentException($"Employer with ID {create.EmployerId} does not exist.");
        }
